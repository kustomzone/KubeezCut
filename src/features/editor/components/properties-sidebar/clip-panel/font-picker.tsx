...
export function FontPicker({
  ...
  /** Open panel (search + preview + list) — hit-testing for scrollbar / odd targets. */
  ...
  useEffect(() => {
    ...
    /**
     * Outside-close must run in the capture phase so we see the real `target` before
     * focus/blur side effects. Scrollbar drags often report a target outside the picker
     * subtree; fall back to panel bounds with padding for hit-testing.
     */
  ...
  return (
    <div ref={rootRef} className="w-full">
      ...
      {open ? (
        <div
          ref={popoverPanelRef}
          className="mt-2 space-y-2 rounded-md border bg-popover p-2 shadow-sm"
        >
          ...
          <div
            ...
            onPointerDownCapture={(event) => {
              // Scrollbar hits use the scroll container as target; stop bubbling so outside-close
              // listeners (bubble phase) never run. Font rows are descendants — target !== currentTarget.
