 ...
 */
export const PropertiesSidebar = memo(function PropertiesSidebar({ fillContainer = false }: PropertiesSidebarProps) {
  ...
  const outerWidthStyle = !rightSidebarOpen
    ? { transition: 'width 200ms' as const }
    : fillContainer
      ? undefined
      : { width: rightSidebarWidth, transition: isResizingRef.current ? 'none' : 'width 200ms' };
  const innerWidthStyle = fillContainer ? undefined : { width: rightSidebarWidth };
    ...
    <>
      ...
      <div
        className={`panel-bg border-l border-border shrink-0 relative h-full ${
          rightSidebarOpen ? (fillContainer ? 'flex-1 min-w-0 w-full' : '') : 'w-0'
        }`}
        style={outerWidthStyle}
      >
        {/* Use Activity for React 19 performance optimization */}
        <Activity mode={rightSidebarOpen ? 'visible' : 'hidden'}>
          <div className="h-full flex flex-col w-full" style={innerWidthStyle}>
            {/* Sidebar Header */}
            <div
        ...
        {/* Resize Handle */}
        {rightSidebarOpen && !fillContainer && (
          <div
