...
/** Master meter (timeline panel): scale labels 0–100% — matches `linearLevelToPercent` mapping. */
export const AUDIO_METER_PERCENT_SCALE_MARKS = [0, 25, 50, 75, 100] as const;

export function percentMarkToPercent(mark: number): number {
  return mark;
}

/** Peak readout next to timeline — same scale as the bar (quiet floor → 100%). */
export function formatMeterPercent(level: number): string {
  if (level <= 0.000001) {
    return '0%';
  }
  const p = linearLevelToPercent(level);
  return `${Math.round(p)}%`;
}

/** Legacy dB tick positions (same mapping as `linearLevelToPercent`). */
