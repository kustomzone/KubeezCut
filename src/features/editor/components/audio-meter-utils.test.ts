...
describe('audio meter utils', () => {
  ...
  it('formatMeterPercent maps linear level to 0–100% readout', () => {
    expect(formatMeterPercent(0)).toBe('0%');
    expect(formatMeterPercent(1e-9)).toBe('0%');
    expect(formatMeterPercent(1)).toBe('90%');
  });
