...
export const PreviewArea = memo(function PreviewArea({ project }: PreviewAreaProps) {
  ...
  const programMonitorFullscreenRef = useRef<HTMLDivElement>(null);
  const { isFullscreen, toggleFullscreen } = useFullscreen({ containerRef: programMonitorFullscreenRef });
  ...
  return (
    <div ref={splitContainerRef} className="flex-1 flex min-h-0 min-w-0 relative" role="region" aria-label="Preview area">
      {sourcePreviewMediaId && (
        ...
        <div
          ref={programMonitorFullscreenRef}
          className={`flex flex-col min-w-0 min-h-0 panel-bg ${hasSidePanels ? '' : 'flex-1'} ${isFullscreen ? 'min-h-screen rounded-none border-0 bg-background' : ''}`}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          ...
          {isPenModeActive ? (
            <div
              className="flex items-center px-3 flex-shrink-0 gap-3 overflow-hidden"
            ...
            <div
              className="@container relative flex flex-shrink-0 items-center overflow-hidden px-3"
              style={{ height: EDITOR_LAYOUT_CSS_VALUES.previewControlsHeight }}
            >
              ...
              <div className="ml-auto flex flex-shrink-0 items-center gap-0.5">
                ...
                <PreviewAspectRatioMenu width={width} height={height} />
