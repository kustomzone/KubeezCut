...
const KEY_BASE_CLASSES =
  'flex h-[2.45rem] items-center justify-center rounded-md border text-[10px] font-medium tracking-[0.06em] shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_6px_16px_rgba(0,0,0,0.2)] transition-[background-color,border-color,color,box-shadow] duration-150 ease-out motion-reduce:transition-none select-none';
...
function KeyboardPreview({
  ...
  const renderRowPair = (pair: KeyboardRowPair, index: number) => (
    <div key={index} className="flex gap-2">
      ...
      <div className="flex min-w-0 flex-[15] gap-[3px]">
        {renderRow(pair.main)}
      </div>
      {/* Navigation / arrow cluster */}
      <div className="flex min-w-0 flex-[3] gap-[3px]">
  ...
  return (
    <div className="overflow-x-auto pb-0.5">
      <div className="mx-auto min-w-[720px] max-w-[900px] space-y-1">
...
export function HotkeyEditor() {
  ...
  const exportHotkeys = () => {
    try {
      ...
      const fileName = `kubeez-hotkeys-${exportDocument.exportedAt.slice(0, 10)}.json`;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-[radial-gradient(circle_at_top,rgba(255,140,58,0.14),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0))]">
      <input
        ref={fileInputRef}
        type="file"
      ...
      <div className="shrink-0 px-3 pb-2 md:px-4">
        <div className="flex min-h-0 max-h-[min(300px,38vh)] overflow-hidden rounded-lg border border-white/7 bg-[#0d0d0f]/90">
          {/* Section layers sidebar */}
          <div className="flex min-h-0 shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-white/6 p-2">
            <button
              type="button"
          ...
          {/* Keyboard */}
          <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-center overflow-y-auto overflow-x-auto p-3 md:p-4">
            <KeyboardPreview
          ...
          <div className="w-[240px] shrink-0 space-y-2 overflow-y-auto border-l border-white/6 p-3 pt-4 md:w-[260px]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain border-t border-white/8 px-3 py-2 md:px-4">
        <div className="columns-[220px] gap-x-2 gap-y-0">
