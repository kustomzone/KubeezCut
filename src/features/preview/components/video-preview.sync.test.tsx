...
vi.mock('@/features/preview/deps/player-core', async () => {
  ...
  >(({ children, width, height, onFrameChange }, ref) => {
    ...
    const onFrameChangeRef = React.useRef<((frame: number) => void) | undefined>(onFrameChange);
...
describe('VideoPreview sync behavior', () => {
  ...
  it('refreshes stale resolved media URLs after blob URL invalidation', async () => {
    ...
    const media = {
      ...
      createdAt: Date.now(),
      updatedAt: Date.now(),
    } as unknown as ReturnType<typeof useMediaLibraryStore.getState>['mediaItems'][number];
