...
describe('TrackRowFrame', () => {
  it('renders a dotted divider for every wrapped row when multiple lanes, including the last row', () => {
    render(
      <>
        <TrackRowFrame multipleLanes>
          <div>Video</div>
        </TrackRowFrame>
        <TrackRowFrame multipleLanes>
          <div>Audio</div>
    ...
    expect(screen.getAllByTestId('track-row-divider')[0]!.className).toContain('border-dotted');
  });

  it('renders no lane dividers when multipleLanes is false', () => {
    render(
      <TrackRowFrame multipleLanes={false}>
        <div>Track</div>
      </TrackRowFrame>
    );

    expect(screen.queryByTestId('track-row-divider')).toBeNull();
    expect(screen.queryByTestId('track-row-top-divider')).toBeNull();
  });

  it('anchors the divider to the row and keeps it out of hit testing', () => {
    const { container } = render(
      <TrackRowFrame multipleLanes showTopDivider>
        <div>Track</div>
      </TrackRowFrame>
    );
    ...
    expect(topDivider.className).toContain('border-dotted');
    expect(divider.className).toContain('pointer-events-none');
    expect(divider.className).toContain('bottom-0');
    expect(divider.className).toContain('border-dotted');
  });

  it('renders a bottom resize handle when track resizing is enabled', () => {
    render(
      <TrackRowFrame multipleLanes onResizeMouseDown={() => undefined} resizeHandleLabel="Resize V1">
        <div>Track</div>
      </TrackRowFrame>
    );
